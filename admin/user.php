<?php
    ...
    $sql_search_users = "SELECT id_user, username FROM tb_user";
    $users = $connection->query($sql_search_users)->fetchAll();
    ...
    if ($_POST) {
        ...
        $sql_search_user = "SELECT username FROM tb_user WHERE username = '$username'";
        $user = $connection->query($sql_search_user)->fetchObject();
        if (!$user) {
            $pass = sha1($pass);
            $sql_insert_user = "INSERT INTO tb_user (id_user, username, password)
                VALUES (null, '$username', '$pass')";
    ...
    if (isset($_GET['excluir'])) {
        $id = $_GET['excluir'];
        $sql_excluir_user = "DELETE FROM tb_user WHERE id_user = '$id'";
        $connection->exec($sql_excluir_user);
        ...
        <nav class="navbar navbar-expand-lg navbar-transparent navbar-absolute fixed-top ">
            <div class="container-fluid">
                <div class="navbar-wrapper">
                    <a class="navbar-brand" href="javascript:;">Gerenciar Usuários</a>
                </div>
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                <form class="form" method="post"
                                    ...
                                    <button type="submit" id="add-shortlink" class="btn btn-success pull-right">
                                        Adicionar
                                    </button>
                                </form>
                            </div>
                ...
                <div class="row">
                    <div class="col-md-8">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <h4 class="card-title">Aqui estão os usuários cadastrados</h4>
                                <table class="table table-hover">
                                    ...
                                    <?php if ($users): ?>
                                        <?php foreach ($users as $user): ?>
                                            <tr>
                                                <td class="col-md-10">
                                                    <a href="redirect.php?short=<?= $user['username'] ?>"><?= $user['username'] ?></a>
                                                </td>
                                                <td class="col-md-2">
                                                    <a class="nav-link"
                                                       onclick="javascript: return confirm('Deseja excluir o usuário?');"
                                                       href="./user.php?excluir=<?= $user['id_user'] ?>">
